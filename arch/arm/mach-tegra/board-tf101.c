...
static struct rfkill_gpio_platform_data tf101_bt_rfkill_pdata[] = {
	{
		.name           = "bt_rfkill",
...
static struct platform_device tf101_bt_rfkill_device = {
	.name = "rfkill_gpio",
 	.id             = -1,
	.dev = {
		.platform_data  = tf101_bt_rfkill_pdata,
	},
 };

 static void __init tf101_bt_rfkill(void)
 {
	 /*Add Clock Resource*/
	 clk_add_alias("bcm4329_32k_clk", tf101_bt_rfkill_device.name, \
				"blink", NULL);
	 return;
 }
